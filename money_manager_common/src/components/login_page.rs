...
use crate::{
    components::buttons::{PrimaryButton, SecondaryLink},
    models::{Coin, User},
    EXCHANGE_RATES, USER,
};

use super::loadings::TextLoading;
...
pub fn LoginPage(cx: Scope) -> Element {
    ...
    let components = if !*is_loading.get() {
        rsx!(
            h1 { "Iniciar Sesión" },
                input {
                    class: "text-black",
                ...
                div {
                    ...
                    PrimaryButton {
                        ...
                        onclick: move |_| {
                            cx.spawn({
                                to_owned![router, set_user, username, is_loading, set_exchanges];
                                async move {
                                    is_loading.set(true);
